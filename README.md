...
群发 / 私发 **本日 / 本周 / 本月** 考勤缺勤/迟到/早退情况。显示姓名、缺勤次数、迟到次数、早退次数（仅显示有异常的人员）。
...
```bash
# 1. 克隆项目
git clone <你的仓库> && cd sign-in-notification-bot

# 2. 配置
cp .env.example .env
# 编辑 .env，填入钉钉应用凭证和 Webhook URL

# 3. 启动（纯调度模式，不占端口 → 只需定时推送）
docker compose up -d

#   如需启用群聊命令回调，取消 docker-compose.yml 中 ports 注释：
#   ports:
#     - "8000:8000"
```
...
如需在群内 @机器人 发 `本日考勤` 并实时回复，需开放端口给钉钉回调：
...
然后去 [钉钉开放平台](https://open.dingtalk.com) → 企业内部应用 → **消息接收模式**，设置：
```
HTTP 地址: https://你的公网域名/dingtalk/callback
```
...
项目包含 `.github/workflows/docker-build.yml`，推送标签 `v*` 或推送到 `main` 分支时自动构建并推送到 Docker Hub。使用前需在仓库 Settings → Secrets 中配置：
...
**Q: 需要开放端口吗？**
A: 不需要。纯调度模式下通过 Webhook 推送，不占端口。只有需要群内命令实时回复时才需开放。
...
**Q: 如何迁移到新服务器？**
A: 复制 `.env` 和项目文件，```docker compose up -d``` 即可。